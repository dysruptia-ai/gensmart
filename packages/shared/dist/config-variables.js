...
export const isValidConfigVariableKey = (key) => typeof key === 'string' &&
    key.length > 0 &&
 ...
 */
export function validateConfigVariableSchema(entry) {
    if (!entry || typeof entry !== 'object')
        return 'errors.configVariables.invalidSchema';
    const s = entry;
    ...
    if (!s.type || !CONFIG_VARIABLE_TYPES.includes(s.type)) {
    ...
    if (typeof s.required !== 'boolean')
        return 'errors.configVariables.invalidRequired';
    if (s.type === 'enum') {
 ...
 */
export function validateConfigVariableValue(schema, value) {
    const isEmpty = value === null || value === undefined || value === '';
    if (schema.required && isEmpty)
        return 'errors.configVariables.required';
    if (isEmpty)
        return null;
    switch (schema.type) {
        case 'string':
        case 'textarea': {
            if (typeof value !== 'string')
                return 'errors.configVariables.invalidType';
        ...
        case 'number': {
            ...
            if (schema.min !== undefined && num < schema.min)
                return 'errors.configVariables.belowMin';
            if (schema.max !== undefined && num > schema.max)
                return 'errors.configVariables.aboveMax';
            return null;
        }
        case 'boolean':
 ...
 */
export function mergeConfigVariablesSchema(templateSchema, overrides) {
    const merged = new Map();
    for (const v of templateSchema ?? [])
        merged.set(v.key, v);
 ...
 */
export function injectConfigVariables(systemPrompt, schema, values) {
    if (!systemPrompt)
        return systemPrompt;
    const schemaByKey = new Map((schema ?? []).map((s) => [s.key, s]));
    return systemPrompt.replace(/\{\{config\.([a-z][a-z0-9_]*)\}\}/g, (match, key) => {
 ...
 */
export function findMissingRequiredConfigVariables(schema, values) {
    return (schema ?? [])
        .filter((s) => s.required)
        .filter((s) => {
 ...
 */
export function injectConfigVariablesDeep(obj, schema, values) {
    if (obj === null || obj === undefined)
        return obj;
    if (typeof obj === 'string') {
        return injectConfigVariables(obj, schema, values);
    }
    if (Array.isArray(obj)) {
        return obj.map((item) => injectConfigVariablesDeep(item, schema, values));
    }
 ...
 */
export function initialConfigVariableValues(schema) {
    const out = {};
