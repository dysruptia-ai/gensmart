...
async function seedTestUsers() {
    ...
    try {
        ...
        const passwordHash = await bcryptjs_1.default.hash(TEST_PASSWORD, BCRYPT_ROUNDS);
        console.log('\nSeeding test users...\n');
        for (const user of testUsers) {
            // Insert organization
            const orgResult = await client.query(`INSERT INTO organizations (name, slug, plan, subscription_status)
         VALUES ($1, $2, $3, 'active')
         ON CONFLICT (slug) DO UPDATE SET plan = EXCLUDED.plan
         RETURNING id`, [user.orgName, user.slug, user.plan]);
            const orgId = orgResult.rows[0].id;
            // Insert user
            await client.query(`INSERT INTO users (organization_id, email, password_hash, name, role, email_verified)
         VALUES ($1, $2, $3, $4, 'owner', true)
         ON CONFLICT (email) DO UPDATE
           SET password_hash = EXCLUDED.password_hash,
               organization_id = EXCLUDED.organization_id,
               name = EXCLUDED.name`, [orgId, user.email, passwordHash, user.userName]);
