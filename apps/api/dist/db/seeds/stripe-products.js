...
async function getOrCreateProduct(name, metadata) {
    ...
    const found = existing.data.find((p) => p.metadata['plan'] === metadata['plan'] && p.name === name);
    if (found) {
...
async function getOrCreatePrice(productId, unitAmount, currency, recurring, metadata, nickname) {
    ...
    const found = existing.data.find((p) => p.metadata['plan'] === metadata['plan'] &&
...
async function main() {
    ...
    const starterMonthly = await getOrCreatePrice(starterProduct.id, 2900, 'usd', { interval: 'month', interval_count: 1 }, { plan: 'starter', interval: 'monthly' }, 'Starter Monthly');
    priceMap.starter.monthly = starterMonthly.id;
    const starterQuarterly = await getOrCreatePrice(starterProduct.id, 7830, 'usd', { interval: 'month', interval_count: 3 }, { plan: 'starter', interval: 'quarterly' }, 'Starter Quarterly (10% off)');
    priceMap.starter.quarterly = starterQuarterly.id;
    const starterYearly = await getOrCreatePrice(starterProduct.id, 27840, 'usd', { interval: 'year', interval_count: 1 }, { plan: 'starter', interval: 'yearly' }, 'Starter Yearly (20% off)');
    priceMap.starter.yearly = starterYearly.id;
    // ── Pro Plan ──────────────────────────────────────────────────────────────
    console.log('\n📦 Pro Plan ($79/mo)');
    const proProduct = await getOrCreateProduct('GenSmart Pro', { plan: 'pro' });
    const proMonthly = await getOrCreatePrice(proProduct.id, 7900, 'usd', { interval: 'month', interval_count: 1 }, { plan: 'pro', interval: 'monthly' }, 'Pro Monthly');
    priceMap.pro.monthly = proMonthly.id;
    const proQuarterly = await getOrCreatePrice(proProduct.id, 21330, 'usd', { interval: 'month', interval_count: 3 }, { plan: 'pro', interval: 'quarterly' }, 'Pro Quarterly (10% off)');
    priceMap.pro.quarterly = proQuarterly.id;
    const proYearly = await getOrCreatePrice(proProduct.id, 75840, 'usd', { interval: 'year', interval_count: 1 }, { plan: 'pro', interval: 'yearly' }, 'Pro Yearly (20% off)');
    ...
    const enterpriseMonthly = await getOrCreatePrice(enterpriseProduct.id, 19900, 'usd', { interval: 'month', interval_count: 1 }, { plan: 'enterprise', interval: 'monthly' }, 'Enterprise Monthly');
    priceMap.enterprise.monthly = enterpriseMonthly.id;
    const enterpriseQuarterly = await getOrCreatePrice(enterpriseProduct.id, 53730, 'usd', { interval: 'month', interval_count: 3 }, { plan: 'enterprise', interval: 'quarterly' }, 'Enterprise Quarterly (10% off)');
    priceMap.enterprise.quarterly = enterpriseQuarterly.id;
    const enterpriseYearly = await getOrCreatePrice(enterpriseProduct.id, 191040, 'usd', { interval: 'year', interval_count: 1 }, { plan: 'enterprise', interval: 'yearly' }, 'Enterprise Yearly (20% off)');
    ...
    const addon500 = await getOrCreatePrice(addonProduct.id, 1000, 'usd', null, { plan: 'addon', interval: 'messages_500' }, '500 Extra Messages - $10');
    priceMap.addons.messages_500 = addon500.id;
    const addon2000 = await getOrCreatePrice(addonProduct.id, 3000, 'usd', null, { plan: 'addon', interval: 'messages_2000' }, '2,000 Extra Messages - $30');
    priceMap.addons.messages_2000 = addon2000.id;
    const addon5000 = await getOrCreatePrice(addonProduct.id, 6000, 'usd', null, { plan: 'addon', interval: 'messages_5000' }, '5,000 Extra Messages - $60');
