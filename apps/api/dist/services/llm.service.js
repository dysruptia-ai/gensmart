 ...
 */
const ANTHROPIC_MODELS_WITHOUT_TEMPERATURE = new Set([
    'claude-sonnet-5',
]);
function anthropicModelSupportsTemperature(model) {
...
function getOpenAIClient(byoKey) {
    return new openai_1.default({ apiKey: byoKey ?? process.env['OPENAI_API_KEY'] });
}
function getAnthropicClient(byoKey) {
    return new sdk_1.default({ apiKey: byoKey ?? process.env['ANTHROPIC_API_KEY'] });
}
...
async function chatOpenAI(params) {
...
async function generatePrompt(description, language = 'en') {
