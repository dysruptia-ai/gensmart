...
exports.BLOCKED_HOST_PATTERNS = exports.MEDIA_VALIDATION_HEAD_TIMEOUT_MS = exports.MEDIA_VALIDATION_CACHE_TTL = exports.MEDIA_LIMITS = void 0;
exports.isHostBlocked = isHostBlocked;
...
exports.MEDIA_VALIDATION_CACHE_TTL = 3600; // 1 hour
exports.MEDIA_VALIDATION_HEAD_TIMEOUT_MS = 8000;
...
function isHostBlocked(hostname) {
    return exports.BLOCKED_HOST_PATTERNS.some((pattern) => pattern.test(hostname));
}
...
const UNTRUSTED_MIME_TYPES = new Set([
...
async function validateMediaUrl(url, type) {
    ...
    const limits = exports.MEDIA_LIMITS[type];
    if (!limits) {
        return { valid: false, error: `Unsupported media type: ${type}`, errorCode: 'WRONG_TYPE' };
    }
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), exports.MEDIA_VALIDATION_HEAD_TIMEOUT_MS);
    ...
    try {
        ...
        return {
            valid: false,
            error: isTimeout ? 'URL validation timed out' : `Could not reach URL: ${err.message}`,
    ...
    const allowedTypes = limits.allowedMimeTypes;
    ...
    let effectiveMimeType = baseMimeType;
    if (type === 'image' && UNTRUSTED_MIME_TYPES.has(baseMimeType)) {
        const rangeController = new AbortController();
        const rangeTimeout = setTimeout(() => rangeController.abort(), exports.MEDIA_VALIDATION_HEAD_TIMEOUT_MS);
        let detected = null;
        try {
            ...
            return {
                valid: false,
                error: isTimeout ? 'Media byte check timed out' : `Could not fetch media bytes: ${err.message}`,
                errorCode: isTimeout ? 'TIMEOUT' : 'HEAD_FAILED',
            };
        }
...
//# sourceMappingURL=media-validator.service.js.map