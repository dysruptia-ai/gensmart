...
exports.captureVariableToolDef = void 0;
exports.buildVariableCaptureInstructions = buildVariableCaptureInstructions;
exports.handleCaptureVariable = handleCaptureVariable;
exports.extractedCapturesToToolCalls = extractedCapturesToToolCalls;
const crypto_1 = __importDefault(require("crypto"));
const database_1 = require("../config/database");
/** Build variable capture instructions to append to the system prompt */
function buildVariableCaptureInstructions(variables) {
    if (!variables.length)
        return '';
...
exports.captureVariableToolDef = {
    name: 'capture_variable',
    description: 'Capture a conversation variable value identified from the user message. Call this whenever you detect a value for a variable to capture.',
    parameters: {
        type: 'object',
        properties: {
            variable_name: {
...
async function handleCaptureVariable(conversationId, variableName, variableValue, agentVariables, io) {
    // Validate variable name exists in agent config
    const varDef = agentVariables.find((v) => v.name.toLowerCase() === variableName.toLowerCase());
    if (!varDef) {
        return {
            success: false,
    ...
    const convResult = await (0, database_1.query)('SELECT id, agent_id, organization_id, contact_id, channel, channel_metadata, captured_variables FROM conversations WHERE id = $1', [conversationId]);
    ...
    await (0, database_1.query)(`UPDATE conversations
     ...
     WHERE id = $2`, [JSON.stringify(updatedVars), conversationId]);
    // Sync to contact
    await syncVariableToContact(conv.organization_id, conv.agent_id, conv.contact_id, conv.channel, conv.channel_metadata, varDef.name, variableValue, conversationId, varDef.mapsTo);
...
async function syncVariableToContact(orgId, agentId, contactId, channel, channelMetadata, varName, varValue, conversationId, mapsTo) {
    ...
    if (mapsTo === 'name' || mapsTo === 'email' || mapsTo === 'phone') {
        ...
        if (namePatterns.includes(nameLower))
            targetField = 'name';
        else if (emailPatterns.includes(nameLower))
            targetField = 'email';
        else if (phonePatterns.includes(nameLower))
            targetField = 'phone';
    }
    if (!contactId) {
        // Auto-create contact
        const phone = channel === 'whatsapp'
            ? channelMetadata['phone'] ?? (targetField === 'phone' ? varValue : null)
            : targetField === 'phone' ? varValue : null;
        const result = await (0, database_1.query)(`INSERT INTO contacts (organization_id, agent_id, name, phone, email, source_channel, funnel_stage, created_at, updated_at)
       ...
       RETURNING id`, [
        ...
        if (newContactId) {
            await (0, database_1.query)('UPDATE conversations SET contact_id = $1 WHERE id = $2', [newContactId, conversationId]);
        }
        return;
    }
    // Update existing contact
    if (targetField === 'name') {
        await (0, database_1.query)('UPDATE contacts SET name = $1, updated_at = NOW() WHERE id = $2', [varValue, contactId]);
    }
    else if (targetField === 'email') {
        await (0, database_1.query)('UPDATE contacts SET email = $1, updated_at = NOW() WHERE id = $2', [varValue, contactId]);
    }
    else if (targetField === 'phone') {
        await (0, database_1.query)('UPDATE contacts SET phone = $1, updated_at = NOW() WHERE id = $2', [varValue, contactId]);
    }
    else {
       ...
       WHERE id = $2`, [JSON.stringify({ [varName]: varValue }), contactId]);
    }
}
/**
 ...
 */
function extractedCapturesToToolCalls(extracted) {
    return extracted.map((e) => ({
        id: `synthetic_capture_${crypto_1.default.randomBytes(6).toString('hex')}`,
        name: 'capture_variable',
        arguments: {
...
//# sourceMappingURL=variable-capture.service.js.map