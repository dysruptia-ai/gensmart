...
exports.connectAndListTools = connectAndListTools;
 ...
 */
async function withMCPSession(serverUrl, operation, extraHeaders) {
    // Validate URL
    let parsed;
    try {
 ...
 */
async function withStreamableHTTPSession(serverUrl, operation, extraHeaders) {
    // Validate URL
    let parsed;
    try {
        parsed = new URL(serverUrl);
 ...
 */
async function connectAndListTools(serverUrl, transport = 'sse', extraHeaders) {
    if (extraHeaders && Object.keys(extraHeaders).length > 0) {
        console.log(`[MCP] Connecting to ${serverUrl} via ${transport} to list tools (extra headers: ${Object.keys(extraHeaders).join(', ')})`);
    }
    else {
 ...
 */
async function executeMCPTool(serverUrl, toolName, toolArguments, transport = 'sse', extraHeaders) {
    if (extraHeaders && Object.keys(extraHeaders).length > 0) {
        console.log(`[MCP] Executing tool "${toolName}" on ${serverUrl} via ${transport} (extra headers: ${Object.keys(extraHeaders).join(', ')})`);
    }
    else {
    ...
    try {
        const result = transport === 'streamable-http'
            ? await withStreamableHTTPSession(serverUrl, async (send) => send('tools/call', { name: toolName, arguments: toolArguments }), extraHeaders)
            : await withMCPSession(serverUrl, async (send) => send('tools/call', { name: toolName, arguments: toolArguments }), extraHeaders);
        const res = result;
        let content = '';
