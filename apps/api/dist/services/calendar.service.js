...
exports.resolveCalendarIds = resolveCalendarIds;
 ...
 */
function resolveCalendarIds(toolConfig) {
    const rawCalIds = toolConfig['calendar_ids'];
    const legacyCalId = toolConfig['calendar_id'];
    return rawCalIds?.length ? rawCalIds : (legacyCalId ? [legacyCalId] : []);
}
async function createCalendar(orgId, data) {
    const { name, agentId = null, timezone = 'UTC', availableDays = [1, 2, 3, 4, 5], availableHours = { start: '09:00', end: '17:00' }, slotDuration = 30, bufferMinutes = 15, maxAdvanceDays = 30, notificationEmail = null, } = data;
    const result = await (0, database_1.query)(`INSERT INTO calendars
...
async function getCalendars(orgId) {
    const result = await (0, database_1.query)('SELECT * FROM calendars WHERE organization_id = $1 ORDER BY created_at DESC', [orgId]);
    return result.rows;
}
async function getCalendar(orgId, calendarId) {
    const result = await (0, database_1.query)('SELECT * FROM calendars WHERE id = $1 AND organization_id = $2', [calendarId, orgId]);
    return result.rows[0] ?? null;
}
async function updateCalendar(orgId, calendarId, data) {
    ...
    const result = await (0, database_1.query)(`UPDATE calendars SET ${setClauses.join(', ')}
     WHERE id = $${idx++} AND organization_id = $${idx}
     RETURNING *`, values);
    return result.rows[0] ?? null;
}
async function deleteCalendar(orgId, calendarId) {
    const result = await (0, database_1.query)('DELETE FROM calendars WHERE id = $1 AND organization_id = $2', [calendarId, orgId]);
...
async function getAvailableSlots(calendarId, date) {
    const calResult = await (0, database_1.query)('SELECT * FROM calendars WHERE id = $1', [calendarId]);
    ...
    const hours = cal.available_hours;
    ...
    const bookedResult = await (0, database_1.query)(`SELECT start_time, end_time FROM appointments
     WHERE calendar_id = $1
       AND (start_time AT TIME ZONE $3)::date = $2::date
       AND status != 'cancelled'`, [calendarId, date, tz]);
    ...
    const available = rawSlots.filter((slot) => {
        ...
        if (date === todayLocalStr && slotStartMin <= nowMinutes)
            return false;
