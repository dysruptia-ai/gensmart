...
async function scoreConversation(conversationId, orgId) {
    // Fetch conversation
    const convResult = await (0, database_1.query)('SELECT id, agent_id, contact_id FROM conversations WHERE id = $1 AND organization_id = $2', [conversationId, orgId]);
    const conv = convResult.rows[0];
    if (!conv) {
        throw new Error(`Conversation not found: ${conversationId}`);
    }
    // Fetch messages
    const messagesResult = await (0, database_1.query)(`SELECT role, content FROM messages
    ...
    // Fetch agent info
    const agentResult = await (0, database_1.query)('SELECT name, variables FROM agents WHERE id = $1', [conv.agent_id]);
    const agent = agentResult.rows[0];
    // Fetch existing contact
    let existingContact = {};
    if (conv.contact_id) {
        const contactResult = await (0, database_1.query)('SELECT name, phone, email, funnel_stage, ai_score, ai_service, custom_variables FROM contacts WHERE id = $1', [conv.contact_id]);
...
- 9-10: Ready to purchase/commit, requested appointment or next steps`;
    ...
    if (conv.contact_id) {
        const contactFunnelStage = existingContact['funnel_stage'] ?? 'lead';
        let newStage = null;
        ...
        const updateParams = [parsed.summary, parsed.score, parsed.service];
        let paramIdx = 4;
        if (newStage) {
        ...
        await (0, database_1.query)(`UPDATE contacts SET ${setClauses.join(', ')}
       WHERE id = $${paramIdx++} AND organization_id = $${paramIdx}`, updateParams);
        ...
        if (Object.keys(parsed.variables).length > 0) {
         ...
         WHERE id = $2`, [JSON.stringify(parsed.variables), conv.contact_id]);
        }
    }
    // Update conversation with ai_score and ai_summary
    await (0, database_1.query)('UPDATE conversations SET ai_score = $1, ai_summary = $2, updated_at = NOW() WHERE id = $3', [parsed.score, parsed.summary, conversationId]);
    return parsed;
}
//# sourceMappingURL=ai-scoring.service.js.map