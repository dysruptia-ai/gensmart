 ...
 */
export async function getWebhookSecretForAgent(agentId: string): Promise<string | null> {
  const result = await query<{ config: Record<string, unknown> }>(
    `SELECT config FROM agent_tools
  ...
  for (const row of result.rows) {
    const cfg = row.config as { webhookSecret_encrypted?: string };
    if (cfg?.webhookSecret_encrypted) {
      try {
        return decrypt(cfg.webhookSecret_encrypted);
      } catch (err) {
        console.error(
          `[mcp-webhook] Failed to decrypt webhookSecret for agent ${agentId}:`,
...
export async function recordDelivery(
  ...
  const result = await query<{ delivery_id: string }>(
    `INSERT INTO mcp_deliveries (delivery_id, agent_id, event)
     VALUES ($1, $2, $3)
     ON CONFLICT (delivery_id) DO NOTHING
     RETURNING delivery_id`,
    [deliveryId, agentId, event]
  );
